...
//! Server task for the STM32WB SPI peripheral.
//!
...
//!
//! As noted in the `stm32wb-spi` driver, the `stm32wb` PAC has decided that all
...
use drv_stm32wb_spi as spi_core;
...
fn main() -> ! {
    ...
    spi.initialize(
        device::spi1::cr1::BR_A::Div16,
        8,
        device::spi1::cr1::BIDIMODE_A::Unidirectional,
        device::spi1::cr1::LSBFIRST_A::Msbfirst,
        device::spi1::cr1::CPHA_A::SecondEdge,
        device::spi1::cr1::CPOL_A::IdleHigh,
    );
    ...
    activate_mux_option(&CONFIG.mux_options[current_mux_index], &sys);
    ...
    fn ready_writey(
        ...
        while rx_count < overall_len {
            ...
            if should_sleep {
                ...
                let _ = sys_recv_closed(&mut [], notifications::SPI_IRQ_MASK, TaskId::KERNEL);
            }
        }
...
fn activate_mux_option(opt: &SpiMuxOption, gpio: &sys_api::Sys) {
...
struct DeviceDescriptor {
    ...
    clock_divider: device::spi1::cr1::BR_A,
}
